...
(defn- normalize-fields-for-join [removed-location query stage-number join]
  (cond
...
(defn- normalize-fields-for-stage [removed-location query stage-number]
  (let [stage (lib.util/query-stage query stage-number)]
    (cond-> query
      ...
      (lib.util/update-query-stage stage-number update :joins
                                   (partial mapv #(normalize-fields-for-join removed-location query stage-number %))))))
...
(mu/defn normalize-fields-clauses :- :metabase.lib.schema/query
  ...
  ([query            :- :metabase.lib.schema/query
   ...
   (reduce (partial normalize-fields-for-stage removed-location) query (range (count (:stages query))))))
